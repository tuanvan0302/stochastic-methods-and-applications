"""Configuration handling for the Gaussian HMM experiment."""
...
# The default path to the YAML configuration file for the Gaussian HMM experiment.
DEFAULT_CONFIG_PATH = Path("configs/hmm.yaml")

# A dataclass representing the configuration for the Gaussian HMM experiment. It contains various parameters and paths used throughout the experiment.
...
class HMMConfig:
    ...
    def transition_matrix_path(self) -> Path:
        return self.tables_dir / self.transition_matrix_file
...
def load_hmm_config(path: str | Path = DEFAULT_CONFIG_PATH) -> HMMConfig:
    ...
    try:
        ...
        raise RuntimeError("PyYAML is required to read configs/hmm.yaml.") from exc
    ...
    return HMMConfig(
        ...
        n_iter=int(hmm.get("n_iter", 300)),
        tol=float(hmm.get("tol", 0.0001)),
        min_covar=float(hmm.get("min_covar", 0.001)),
        algorithm=str(hmm.get("algorithm", "viterbi")),
        init_params=str(hmm.get("init_params", "stmc")),
        params=str(hmm.get("params", "stmc")),
        primary_metric=primary_metric,
        minimum_state_share=float(selection.get("minimum_state_share", 0.01)),
        walk_forward_train_mode=str(walk_forward.get("train_mode", "expanding")),
        model_dir=_resolve_path(project_root, outputs.get("model_dir", "models/gaussian_hmm")),
        tables_dir=_resolve_path(project_root, outputs.get("tables_dir", "reports/tables/gaussian_hmm")),
        figures_dir=_resolve_path(project_root, outputs.get("figures_dir", "reports/figures/gaussian_hmm")),
        best_model_file=str(outputs.get("best_model_file", "best_hmm.pkl")),
        ...
        transition_matrix_file=str(outputs.get("transition_matrix_file", "transition_matrix.csv")),
        empirical_transition_matrix_file=str(outputs.get("empirical_transition_matrix_file", "empirical_transition_matrix.csv")),
